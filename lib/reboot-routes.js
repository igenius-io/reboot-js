...
function init(app, rebootConfig) {
    ...
    app.use('/css', express.static(__base + '/' + rebootConfig['reboot-www-directory'] + '/www/css'));
    app.use('/images', express.static(__base + '/' + rebootConfig['reboot-www-directory'] + '/www/images'));
    app.use('/js', express.static(__base + '/' + rebootConfig['reboot-www-directory'] + '/www/js'));
    app.use('*/app.js', function(req, res, next) {
        res.sendFile(path.join(__dirname  + '/' + rebootConfig['reboot-apps-directory'] + '/' + req.params[0] + '/app.js'));
    });
